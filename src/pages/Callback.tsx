...
import { exchangeCode, createSession } from '../services/backendApi'

type Step = 'exchanging' | 'creating-session' | 'done' | 'error'

export default function Callback() {
  const navigate = useNavigate()
  const [step, setStep] = useState<Step>('exchanging')
  const [errorMsg, setErrorMsg] = useState('')
  const ran = useRef(false) // StrictMode guard — prevent double execution
  ...
  const label = step === 'exchanging' ? 'Connecting to Spotify…' : 'Creating session…'
